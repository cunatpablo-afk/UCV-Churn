...
RUN_SUFFIX = "it_3a"
DATASET_PATH = MODELING_DIR / "churn_modeling_dataset_it_3a.csv"
...
def export_feature_importance() -> dict[str, Path]:
    ...
    if not MODEL_PATH.exists():
        raise FileNotFoundError(f"No existe {MODEL_PATH}. Ejecuta primero train_models_it_3.py.")
    ...
    top75_path = MODEL_REPORTS_DIR / f"top_features_75_{RUN_SUFFIX}.csv"

    importance.to_csv(importance_path, index=False)
    grouped.to_csv(grouped_path, index=False)
    grouped.head(75).to_csv(top75_path, index=False)

    return {
        "feature_importance": importance_path,
        "feature_importance_grouped": grouped_path,
        "top_features_75": top75_path,
    }
