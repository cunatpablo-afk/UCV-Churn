...
## Iteracion 3D:
## Dataset Top50 dentro de 3A, como version mas compacta que 3C Top75.
## No hay tuning: se mantienen hiperparametros de 3C para aislar el efecto de
## reducir de 75 a 50 variables.

REDUCED_DATASET = MODELING_DIR / "churn_modeling_dataset_it_3d_top50.csv"

base.RUN_SUFFIX = "it_3d_top50"
...
def load_modeling_dataset() -> pd.DataFrame:
    base.log(f"Cargando dataset Top50 de modelado: {REDUCED_DATASET}")
    if not REDUCED_DATASET.exists():
        raise FileNotFoundError(
            f"No existe {REDUCED_DATASET}. Ejecuta primero src/features/build_reduced_dataset_it_3d_top50.py."
        )
    ...
    base.log(f"Dataset Top50 cargado: {len(df):,} filas, {df.shape[1]:,} columnas")
    return df
